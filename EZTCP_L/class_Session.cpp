	...
	_s(socket(AF_INET, SOCK_STREAM, NULL))
{
	...
	_s_addr.sin_addr.s_addr = inet_addr(ip_ps);
...
eztcp::Session::Session(SOCKET s_listening, int buffer_size) :
	_s(accept(s_listening, reinterpret_cast<sockaddr*>(&_s_addr), &socket_size))
{
	if (_s == INVALID_SOCKET)
		throw SocketError(WSAGetLastError());
	_buffer_p = new char[buffer_size];
...
void eztcp::Session::send(FilePack& package)
{
	if (send_header(_s, &package._size_left) == SOCKET_ERROR)
		throw SocketError(WSAGetLastError());
	while (package._size_left > 0x40000000)
	{
		if (sendfile(_s, package._file_h, NULL, 0x40000000) == -1)
			throw SocketError(WSAGetLastError());
		package._size_left -= 0x40000000;
	}
	if (sendfile(_s, package._file_h, NULL, package._size_left) == -1)
		throw SocketError(WSAGetLastError());
	package._size_left = 0;
}
